...
class Dataset(object):
    ...
    def __init__(self, path, reg_data):
        ...
        if(FLAGS.dataset!='yelp' and FLAGS.dataset!='music'):
        ...
        # np.save("train_user_ft.npy",self.trainMatrix.toarray())
    ...
    def get_idx(self, num_user, all_user, init):
        ...
        print(len(cur_set))
        t=np.concatenate([np.random.choice(cur_set, num_user-len(must), replace=False),np.array(list(must),dtype=np.int)])
        print("test",len(t))
        return t
